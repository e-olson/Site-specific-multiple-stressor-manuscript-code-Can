...
import pandas as pd
from Tools import commonfxns as cf, OAPBuoyData as OAP, OAPBuoyComp as bc,viz, evalfxns as ev, frequencyBands as fb
...
vlist0=['tos','sos','phos','spco2','o2os','chlos','l10chlos','hplus','phosC','talkos','omega_a_0',
          'o2percos','co2dryair','AOUos','dissicos','apco2']
def loadDetrendAnomObs(icomp,deseas=False):
    ...
    ott,oval,tday=cf.fillblank(ott0,oval0) # fill missing values with NaNs
    ott,oval_b,tday=cf.fillblank(ott0,oval0_b)
...
def calcbands(Tvec=[7,31,365],freq='daily',cpath='/work/ebo/calcs/buoyCompTS/GFDL-ESM4.1.1975_2022/comps/',opt=None,vlist=None): #bc.compsaveloc):
    if vlist is None:
        vlist=vlist0
    ...
    labelStr[0]=f"1 to {Tvec[0]} days"
    for ii in range(1,len(Tvec)):
        labels[ii]=f"{Tvec[ii-1]}_{Tvec[ii]}"
        labelStr[ii]=f"{Tvec[ii-1]} to {Tvec[ii]} days"
    labels[len(Tvec)]=f"g{Tvec[-1]}"
    labelStr[len(Tvec)]=f"> {Tvec[-1]} days"
...
if __name__=="__main__":
    ...
    bc.compileStats(save=True,path='/work/Elise.Olson/calcs/buoyCompTS/GFDL-ESM4.1.1975_2022/comps/')
    calcbands()

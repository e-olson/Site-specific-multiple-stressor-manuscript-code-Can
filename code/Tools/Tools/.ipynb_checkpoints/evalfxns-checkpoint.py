...
import commonfxns as cf, OAPBuoyData as OAP
...
defaultSaveLoc=    '/space/hall5/sitestore/eccc/crd/ccrn/users/reo000/work/calcs/timeSeriesComps/'
# locFinalFigs='/work/Elise.Olson/calcs/evalFigs/' # location to save figs for presentation
locFinalFigs='/space/hall5/sitestore/eccc/crd/ccrn/users/reo000/work/figs/'
...
class timeSeriesComp:
    ...
    def calc_stats(self):
        ...
        obs_SeasAmp=np.nanmax(self.obs_gsmooth)-np.nanmin(self.obs_gsmooth) \
                    if np.sum(~np.isnan(self.obs_gsmooth))>300/365*self.yrfacm else np.nan
        obs_SeasAmp_b=np.nanmax(self.obs_gsmooth_b)-np.nanmin(self.obs_gsmooth_b) \
                    if np.sum(~np.isnan(self.obs_gsmooth_b))>300/365*self.yrfacm else np.nan
        ...
        self.stats={'bias':bias,'bias_b':bias_b,'mod_FullMean':mod_FullMean,'mod_SeasAmp':mod_SeasAmp,'obs_SeasAmp':obs_SeasAmp,
                    'obs_SeasAmp_b':obs_SeasAmp_b,'seas_AmpDiffPerc':seas_AmpDiffPerc,
    ...
    def statsSummary(self):
        ...
        sd['mod_opt_coef0']=self.modfits['optfit'].coef[0] if ( 'optfit' in self.modfits.keys() and self.modfits['optfit'] is not None) else np.nan
        sd['obs_opt_coef0']=self.obsfits['optfit'].coef[0] if ( 'optfit' in self.obsfits.keys() and self.obsfits['optfit'] is not None) else np.nan
        sd['mod_opt_coef1']=self.modfits['optfit'].coef[1] if ( 'optfit' in self.modfits.keys() and self.modfits['optfit'] is not None) else np.nan
        sd['obs_opt_coef1']=self.obsfits['optfit'].coef[1] if ( 'optfit' in self.obsfits.keys() and self.obsfits['optfit'] is not None) else np.nan
